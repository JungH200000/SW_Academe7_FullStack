...
//---post 단건 가져오기 (spring과 연동시) -----------------------------
export const apiFetchPostByIdWithSpring = async (id) => {
    const response = await axiosInstance.get(`/posts/${id}`);
    return response.data; //PostDTO 객체로 응답 {id:1,title:'aaa',...}
};
//---post 단건 가져오기 (node.js와 연동시) -----------------------------
export const apiFetchPostById = async (id) => {
    const response = await axiosInstance.get(`/posts/${id}`);
    const data = response.data?.data;
    if (data && data.length > 0) {
        return data[0]; //[ {id:1,title:'aaa',...} ]
    }
    return null;
};
//---post글 수정하기 -----------------------------------
